...
export default function Profile() {
  ...
  const rows = [
    { icon: 'call' as const, label: 'Phone', value: user?.phone },
    { icon: 'mail' as const, label: 'Email', value: user?.email ?? 'Not set' },
    { icon: 'globe' as const, label: 'Language', value: user?.locale?.toUpperCase() },
    { icon: 'shield-checkmark' as const, label: 'Account', value: user?.status },
  ];
  ...
  return (
    <Screen>
      <ScrollView contentContainerStyle={{ padding: space(5), gap: space(5) }}>
        ...
        <Button title="Log out" variant="danger" icon="log-out" onPress={async () => { await logout(); router.replace('/login'); }} />
      </ScrollView>
    </Screen>
  );
}
