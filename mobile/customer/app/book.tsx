...
function Option({ active, children, onPress }: { active: boolean; children: ReactNode; onPress: () => void }) {
  return (
...
export default function Book() {
  ...
  async function confirm() {
    ...
    try {
      ...
      router.replace(`/booking/${r.data.id}`);
    } catch (e: any) {
  ...
  return (
    <Screen>
      <View style={{ flexDirection: 'row', alignItems: 'center', padding: space(4), gap: space(3) }}>
        <Pressable onPress={() => router.back()}><Ionicons name="close" size={26} color={colors.text} /></Pressable>
...
const styleSection = { fontSize: 12, fontWeight: '700' as const, color: colors.muted, textTransform: 'uppercase' as const, letterSpacing: 0.5 };
