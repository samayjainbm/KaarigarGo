...
export default function Login() {
  ...
  async function send() {
    ...
    setLoading(true);
    try {
      const r = await Api.requestOtp(phone.trim(), 'WORKER');
  ...
  async function verify() {
    ...
    setLoading(true);
    try {
      const r = await Api.verifyOtp(phone.trim(), code.trim(), 'WORKER');
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <View style={{ flex: 1, padding: space(6), paddingTop: space(20), justifyContent: 'flex-end' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: space(2), marginBottom: space(6) }}>
            ...
            <Text style={{ color: '#fff', fontSize: 22, fontWeight: '800' }}>KaarigarGo Pro</Text>
          </View>
          <Text style={{ color: '#fff', fontSize: 30, fontWeight: '800' }}>Earn on your terms.</Text>
          <Text style={{ color: '#c7d2fe', fontSize: 15, marginTop: space(2) }}>Accept jobs, get paid fast.</Text>
        </View>
        ...
        <View style={{ backgroundColor: colors.bg, borderTopLeftRadius: radius.xl, borderTopRightRadius: radius.xl, padding: space(6), gap: space(4) }}>
          {step === 'phone' ? (
            <>
              <Text style={{ fontSize: 20, fontWeight: '800', color: colors.ink }}>Pro sign in</Text>
              <Field label="Phone number">
                <Input value={phone} onChangeText={setPhone} keyboardType="phone-pad" placeholder="+91…" autoFocus />
              </Field>
              <Text style={{ color: colors.muted, fontSize: 12 }}>Demo pro: +919000000003</Text>
